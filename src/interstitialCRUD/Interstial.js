...
import InterstitialForm, { convertTime } from "./InterstialForm.js";
...
const Interstial = () => {
  ...
  const loadData = async (tableName) => {
    const result = await axios.get(
      `http://142.93.214.96:9090/data/${tableName}/`
    );
  ...
  const enterEdit = (item) => {
    // console.log(convertTime(parseAdjust(item.NominalDuration).getTime()));
    console.log(item);
    setOpenForm(true);
    setEditInterstitial({
      ...item,
